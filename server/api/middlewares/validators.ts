...
import { isFloatNumber, isIntegerNumber, noSpaceString } from "../helpers/helper";
...
const validateAuthToken = (req, res, next) => {
    ...
    if (authHeader) {
        ...
        return res.status(Constants.response.unauthorized).send({
...
const validatePermission = (allowed) => {
    ...
    return (req, res, next) => {
        ...
        if (isAdmin && allowed == 'admin' || !isAdmin && allowed == 'user') {
            ...
            res.status(403).send({ status: 'error', error: 'Unauthorized action' });
        }
    }
};
